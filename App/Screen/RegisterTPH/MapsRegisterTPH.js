...
class MapRegisterTPH extends React.Component {
    constructor(props) {
        super(props);
    ...
    onClickBlok(poly) {
        const location = {
            block_name: poly.block_name,
            blockcode: poly.werks_afd_block_code.substring(poly.werks_afd_block_code.length - 3, poly.werks_afd_block_code.length),
            afd_code: poly.werks_afd_block_code.substring(4, 5),
            werks: poly.werks_afd_block_code.substring(0, 4)
        }
            ...
            <View style={styles.container}>
                ...
                >
                    ...
                    {this.state.poligons.length > 0 && this.state.poligons.map((poly, index) => {
                        ...
                        const location = {
                            block_name: poly.blokname,
                            werks_afd_block_code: poly.werks_afd_block_code
                        }
                        return (
                            <View key={index}>
                                <Polygon
                                    ...
                                    strokeWidth={3}
                                    tappable={true}
                                    onPress={() => this.onClickBlok(location)}
                                />
                                <Marker
                ...
                <View style={{ position: 'absolute', top: 0, width: '100%', height: 82, backgroundColor: 'white', paddingHorizontal: 16, paddingVertical: 12 }}>

                    <Card style={{ width: '100%', justifyContent: 'space-around', height: 50, borderRadius: 10, borderWidth: 1, borderColor: Colors.tintColorPrimary }}>
                        <TouchableOpacity
                            activeOpacity={0.2} onPress={() => this.props.navigation.navigate("SearchBlok")} style={{ width: '100%', justifyContent: 'space-around', height: 50, borderRadius: 10, borderWidth: 1, borderColor: Colors.tintColorPrimary }}>
                            <Text style={{
                                marginLeft: 16,
                                fontSize: 14,
                                color: Colors.tintColorPrimary,
